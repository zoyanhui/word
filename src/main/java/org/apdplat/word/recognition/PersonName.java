...
public class PersonName {
    ...
    private static List<Word> selectBest(List<List<Word>> candidateWords) {
        ...
        for (List<Word> candidateWord : candidateWords) {
            if (LOGGER.isDebugEnabled()) {
            ...
            for (Word word : candidateWord) {
                seq.append(word.getPartOfSpeech().getPos().charAt(0)).append(" ");
            }
            ...
            for (String pos_seq : POS_SEQ.keySet()) {
...
//        });
        //选择分值最高的
        ArrayList<Map.Entry<List<Word>, Integer>> entries = Lists.newArrayList(map.entrySet());
        Collections.sort(entries, new Comparator<Map.Entry<List<Word>, Integer>>() {
            @Override
            public int compare(Map.Entry<List<Word>, Integer> a, Map.Entry<List<Word>, Integer> b) {
                return b.getValue().compareTo(a.getValue());
            }
        });
        List<Word> result = entries.get(0).getKey();
